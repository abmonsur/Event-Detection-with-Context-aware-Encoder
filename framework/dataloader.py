...
class ACETriDataset(Dataset):
    ...
    def collate_fn(self, data):
        sentence_ids, input_ids, input_masks, in_sent, segment_ids, labels, ners, sentence = [], [], [], [], [], [], [], []

        for item in data:
            # Handle sentence_ids
            sentence_ids.append(torch.tensor(item['sentence_ids']))

            # Handle input_ids
            l = len(item['input_ids'])
            for i in range(l):
                if item['input_ids'][i] == None:
                    item['input_ids'][i] = 0
            input_ids.append(torch.tensor(item['input_ids']))

            # Handle input_masks
            input_masks.append(torch.tensor(item['input_masks']))

            # Handle in_sent
            in_sent.append(torch.tensor(item['in_sent']))

            # Handle segment_ids
            segment_ids.append(torch.tensor(item['segment_ids']))

            # Handle labels
            labels.append(torch.tensor(item['labels']))

            # Handle ners
            ners.append(torch.tensor(item['ners']))
...
class ACETriDataloder(Dataset):
    def __init__(self, config, i):
        ...
        #print(config.llama_path, type(config.llama_path))
        self.tokenizer = AutoTokenizer.from_pretrained(config.llama_path)
    ...
    def read_sample(self, index, sample, type):
        ...
        for i, asample in enumerate(sample):
            ...
            # sentence
            tokens.append("<|begin_of_text|>")  # LLaMA start-of-sequence token
            ...
            for (i, token) in enumerate(sentence):
                ...
                if len(atoken) == 0:
                    tokens.append('<unk>')  # LLaMA unknown token
                else:
            ...
            tokens.append("<|end_of_text|>")  # LLaMA end-of-sequence token
            labels.append(self.vocab2index['None'])
            in_sent.append(0)
            ners.append(self.ner2id['None'])
            ...
            while len(input_ids) < self.max_sentence_length:
                input_ids.append(self.tokenizer.pad_token_id)  # Use LLaMA's padding token ID
    ...
    def read_pred_sample(self, file):
        ...
        for i, asample in enumerate(data):
            ...
            tokens.append("<|begin_of_text|>")  # LLaMA start-of-sequence token
            in_sent.append(0)  # Optional, as LLaMA doesn't require in_sent or segment_ids
            # LLaMA does not use segment IDs, so this can be omitted or set to a single value
            segment_ids.append(0)  # Optional placeholder for compatibility

            # Tokenize each word in the sentence
            for (i, token) in enumerate(sentence):
                atoken = self.tokenizer.tokenize(token)  # Tokenize with LLaMA's tokenizer
                if len(atoken) == 0:
                    tokens.append('<unk>')  # LLaMA unknown token
                else:
                    tokens.append(atoken[0])  # Take the first subword token
                in_sent.append(1)  # Optional, used for tracking in-sentence positions
                segment_ids.append(0)  # Segment IDs are not needed in LLaMA

            # Add end-of-sequence token
            tokens.append("<|end_of_text|>")  # LLaMA end-of-sequence token
            in_sent.append(0)  # Optional, only for compatibility
            segment_ids.append(0)  # Optional placeholder

            # Convert tokens to IDs using LLaMA tokenizer
            input_ids = self.tokenizer.convert_tokens_to_ids(tokens)
    ...
    def read_pred_ner_sample(self, file):
        ...
        for i, asample in enumerate(data):
            ...
            # Add start-of-sequence token
            tokens.append("<|begin_of_text|>")  # LLaMA start-of-sequence token
            in_sent.append(0)  # Optional, may not be needed
            segment_ids.append(0)  # Optional placeholder; LLaMA does not use segment IDs

            # Tokenize the sentence
            for (i, token) in enumerate(sentence):
                atoken = self.tokenizer.tokenize(token)  # Tokenize with LLaMA tokenizer
                if len(atoken) == 0:
                    tokens.append('<unk>')  # LLaMA unknown token
                else:
                    tokens.append(atoken[0])  # Use the first subword token
                in_sent.append(1)  # Optional, tracking in-sentence position
                segment_ids.append(0)  # Optional, set to 0 as LLaMA doesn't require segment IDs

            # Add end-of-sequence token
            tokens.append("<|end_of_text|>")  # LLaMA end-of-sequence token
            in_sent.append(0)  # Optional
            segment_ids.append(0)  # Optional placeholder

            # Convert tokens to IDs using the LLaMA tokenizer
            input_ids = self.tokenizer.convert_tokens_to_ids(tokens)

            # Create attention masks (1 for actual tokens, 0 for padding)
            input_masks = [1] * len(input_ids)

            # Add padding if input length is less than the maximum sentence length
            while len(input_ids) < self.max_sentence_length:
                input_ids.append(self.tokenizer.pad_token_id)  # Use LLaMA padding token ID
                input_masks.append(0)  # Mask out the padded positions
                in_sent.append(0)  # Optional
                segment_ids.append(0)  # Optional placeholder
    ...
    def read_args_sample(self, index, sample, type):
        ...
        for i, asample in enumerate(sample):
            ...
            in_sent.append(0)  # Optional
            segment_ids.append(0)  # Optional, LLaMA does not use segment IDs

            # Tokenize the sentence
            for (i, token) in enumerate(sentence):
                atoken = self.tokenizer.tokenize(token)  # Tokenize using LLaMA tokenizer
                if len(atoken) == 0:
                    tokens.append('<unk>')  # LLaMA unknown token
                else:
                    tokens.append(atoken[0])  # Use the first subword token
                in_sent.append(1)  # Optional
                segment_ids.append(0)  # Optional, LLaMA doesn't require segment IDs
                if i in offset2args:
