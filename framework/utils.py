...
def trigger_combine_event(old_data, new_data):
    if len(new_data) == 0:
        return old_data
    init = False #init is a flag indicating whether old_data was empty initially.
    res = []
    if len(old_data) == 0:
        init = True
    ...
    for old_sample_index in range(len(old_data)-1, -1, -1):
        ...
        combine_flag = False
        for new_sample_index in range(len(new_data)-1, -1, -1): # To find matching instances in old_data and new_data.
            new_sample = new_data[new_sample_index]
            if old_sample['input_ids'] == new_sample['input_ids']: # it indicates they correspond to the same instance in terms of input data.
                ...
                if len(eqoffset) > 0:
                    ...
                    for i in eqoffset: 
                        if old_sample['labels'][i] != new_sample['labels'][i]:
                            ...
                            eqflag = True             
                    if eqflag == False: #if same then remove the new sample, If no conflict, combine labels from new_sample into old_sample
        ...
        if (combine_flag and init) or (init == False):
            temp = copy.deepcopy(old_sample) #copy.deepcopy creates a deep copy of the list. This ensures that the newly created list is independent of the original arrays and that any modifications to it do not affect the original lists.
            res.append(temp)
    res += new_data
    return res
